...
import { CheckoutItem, CheckoutItemSkeleton, CheckoutSidebar, Container, Title, WhiteBlock } from "@/components/shared";
        ...
        <div className="flex flex-col gap-10 flex-1 mb-20">
          <WhiteBlock title="1. Cart">
            <div className="flex flex-col gap-5">
              {loading && Array.from({ length: 3 }).map((_, index) => <CheckoutItemSkeleton key={index} className="h-15 w-full" />)}
              {!loading &&
                items.map((item) => (
                  <CheckoutItem
                    key={item.id}
          ...
          <WhiteBlock title="3. Delivery address">
            <div className="flex flex-col gap-5">
              ...
              <Textarea className="text-base" rows={5} placeholder="Comment..." />
            </div>
          </WhiteBlock>
        </div>
